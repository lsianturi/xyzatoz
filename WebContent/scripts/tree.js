...
function ttmpR(ttmpS, ttmpT) {
	if (Boolean(this.n_state & 8) != Boolean(ttmpS))
		return;
	var ttmpU = (this.a_config[2] ? this.a_config[2][ttmpS ? 'hc' : 'ho']
			: null);
	ttmpU = (ttmpU ? ttmpU : this.o_root.a_tpl[ttmpS ? 'onItemClose'
			: 'onItemOpen']);
	if (ttmpU) {
	...
	if (B_DOM) {
		...
		if (!ttmpV.innerHTML)
			ttmpV.innerHTML = this.ttmpX();
		ttmpV.style.display = (ttmpS ? 'none' : 'block')
	} else if (!ttmpT)
		window.location = window.location
}
function ttmpY(ttmpZ) {
	var ttmpU = (this.a_config[2] ? this.a_config[2][ttmpZ ? 'hd' : 'hs']
			: null);
	ttmpU = (ttmpU ? ttmpU : this.o_root.a_tpl[ttmpZ ? 'onItemDeselect'
			: 'onItemSelect']);
	if (ttmpU) {
		eval('var ttmpO=' + ttmpU + '(this);');
		if (!ttmpO)
	...
	window
			.setTimeout(
					"window.status=unescape('"
							+ (ttmpe ? ''
									: (this.a_config[2]
											&& this.a_config[2]['sb'] ? escape(this.a_config[2]['sb'])
...
function ttmpQ() {
	...
	this.n_state = (this.n_depth ? 0 : 32) + (this.a_children ? 16 : 0)
	...
	var ttmpj = (this.o_root.a_tpl['style_icons'] ? ' class="' + this.o_root.a_tpl['style_icons'] + '"'
			: ''), ttmpk = [], ttmpl = this.o_parent, ttmpm = this.a_config[2];
	for ( var i = this.n_depth; i > 1; i--) {
		ttmpk[i] = '<img src="'
				+ this.o_root.a_tpl[ttmpl.n_state & 1 ? 'icon_e' : 'icon_l']
				+ '"' + ttmpj + ' border="0">';
		ttmpl = ttmpl.o_parent
	}
	this.load();
	var ttmpn = this.state_lookup(true);
	return '<table cellpadding="0" cellspacing="0" border="0"><tr onmouseover="TREES['
			+ this.o_root.n_id
			+ '].mover('
			+ this.n_id
			+ ')" onmouseout="TREES['
			...
			+ ')"><td nowrap>'
			+ ttmpk.join('')
			+ (ttmpn[1] ? (this.a_children ? '<a href="javascript: TREES['
					+ this.o_root.n_id + '].toggle(' + this.n_id
					+ ')" onmouseover="TREES[' + this.o_root.n_id + '].mover('
					+ this.n_id + ')" onmouseout="TREES[' + this.o_root.n_id
					+ '].mout(' + this.n_id + ')"><img src="' + ttmpn[1]
					+ '" border="0" name="j' + this.o_root.n_id + '_'
					+ this.n_id + '"' + ttmpj + '></a>' : '<img src="'
					+ ttmpn[1] + '" border="0"' + ttmpj + '>') : '')
			+ (ttmpn[0] ? '<a href="'
					+ this.a_config[1]
					+ '" target="'
					+ (ttmpm && ttmpm['tw'] ? ttmpm['tw']
							: this.o_root.a_tpl['target']) + '" title="'
					+ (ttmpm && ttmpm['tt'] ? ttmpm['tt'] : '')
					+ '" onclick="return TREES[' + this.o_root.n_id
					+ '].select(' + this.n_id + ')" ondblclick="TREES['
					+ this.o_root.n_id + '].'
					+ (this.a_children ? 'toggle(' : 'select(') + this.n_id
					+ ')"><img src="' + ttmpn[0] + '" border="0" name="i'
					+ this.o_root.n_id + '_' + this.n_id + '"' + ttmpj
					+ '></a>' : '')
			+ '</td><td nowrap'
			...
			+ this.a_config[1]
			+ '" target="'
			+ (ttmpm && ttmpm['tw'] ? ttmpm['tw'] : this.o_root.a_tpl['target'])
			+ '" title="'
			+ (ttmpm && ttmpm['tt'] ? ttmpm['tt'] : '')
			...
			+ this.o_root.n_id
			+ '].'
			+ (this.a_children ? 'toggle(' : 'select(')
			+ this.n_id
			+ ')">'
			+ this.a_config[0]
			+ '</a></td></tr></table>'
			+ (this.a_children ? '<div id="c' + this.o_root.n_id + '_'
					+ this.n_id + '" style="display:'
					+ (this.n_state & 8 ? 'block">' + this.ttmpX() : 'none">')
					+ '</div>' : '')
}
function ttmpi() {
	var ttmpo = [];
	for ( var i = 0; i < this.a_children.length; i++)
		ttmpo[i] = this.a_children[i].ttmpP();
	return ttmpo.join('')
}
function ttmpg() {
	var ttmpp = Math.floor(this.n_node_id / 31);
	this.o_root.a_states[ttmpp] = (this.n_state & 8 ? this.o_root.a_states[ttmpp]
			| (1 << (this.n_node_id % 31))
			: this.o_root.a_states[ttmpp] & ~(1 << (this.n_node_id % 31)));
	document.cookie = 'tree_' + this.o_root.n_id + '_state='
			+ this.o_root.a_states.join('_')
}
function ttmph() {
	...
	var ttmpp = Math.floor(this.n_node_id / 31);
	if (Boolean(this.o_root.a_states[ttmpp] & (1 << (this.n_node_id % 31))))
		this.n_state |= 8;
	else
		this.n_state &= ~8
}
function ttmpf(ttmpr) {
	...
	if (!ttmpu)
		ttmpu = this.o_root.a_tpl['icon_' + ttmps];
	if (!ttmpu)
		ttmpu = this.o_root.a_tpl['icon_' + (ttmps & ~64)];
	var ttmpv = this.o_root.a_tpl['icon_' + ttmpt];
	var ttmpw = this.a_config[2] ? this.a_config[2]['s' + (ttmps & ~48)] : 0;
	if (!ttmpw)
		ttmpw = this.o_root.a_tpl['style_' + ttmps];
	if (!ttmpw)
		ttmpw = this.o_root.a_tpl['style_' + (ttmps & ~64)];
	if (ttmpr)
		return [ ttmpu, ttmpv, ttmpw ];
	...
	if (ttmpx)
		ttmpx.className = ttmpw
}
var TREES = [], B_DOM;
ttmpW = document.all ? function(ttmpy) {
	return document.all[ttmpy]
} : (document.getElementById ? function(ttmpy) {
