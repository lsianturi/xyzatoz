...
/**
parameter:
s is the float number to be validated

precondition:
before cannot be less than 1
after cannot be less than 0

return
true if validation is okay
false if validation is not okay

method invocation example:
isFloatSpecifiedLength(check, 2, 2)

check = 22.2222 would return false

check = 222.2 would return false

check = 2.2 would return true
*/
function isFloatSpecifiedLength(s, before, after){
    var i;
    var seenDecimalPoint = false;

    if (eval(s.length) > (eval(before) + eval(after) + 1)){ 
		return false;
	}
    ...
    {   
		...
		if (((c == decimalPointDelimiter)||(c == decimalPointDelimiter2)) && !seenDecimalPoint && (i==0)) return false;
		if (((c == decimalPointDelimiter)||(c == decimalPointDelimiter2)) && !seenDecimalPoint && (i== (s.length - 1))) return false;		
        
        if (((c == decimalPointDelimiter)||(c == decimalPointDelimiter2)) && !seenDecimalPoint){
			if (i > before) return false;
			if ((s.length - (i + 1)) > after) return false;
        }
	...
	if ((seenDecimalPoint == false)&& (s.length > before)) return false;
