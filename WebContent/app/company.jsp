...
<script type="text/javascript">
	...
	function add() {
		doGoToUrl('<c:url value="/updateCompany.do?dispatch=add" />');
	}
	function del(id) {
		doGoToUrl('<c:url value="/updateCompany.do?dispatch=delete"/>'+'&id='+id);
	}
	function edit(id) {
		doGoToUrl('<c:url value="/updateCompany.do?dispatch=update"/>'+'&id='+id);
	}
</script>
</HEAD>
<BODY>
...
<!-- Help Page Finish -->
<html:form action="/company.do" method="post" >	
	...
	<c:if test="${DataList!=null}">
		<table width="100%" border="0" cellpadding="3" cellspacing="0" class="tblBorder">
			...
			<tr class="tblHeader"> 
				...
				<td><bean:message key="form.company.spm"/></td>
				<td><bean:message key="form.company.prefix"/></td>
			...
			<c:forEach var="comp" items="${DataList.list}" varStatus="status">
				...
				<c:when test="${abs.isManual==1}">
					...
					<td class="celBorder"><c:out value="${comp.name}"/>&nbsp;</td>
					<td class="celBorder"><c:out value="${comp.user.firstName}"/>&nbsp;<c:out value="${comp.user.lastName}"/></td>
					<td class="celBorder"><c:out value="${comp.prefix}"/>&nbsp;</td>
